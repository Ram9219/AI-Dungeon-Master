// ======== CONFIGURATION ======== //
const GEMINI_API_KEY = "AIzaSyBESPekFRljFK1_4IvNoW8nKMjPvGJaJjk"; // Replace with your actual API key
const AI_MODEL = "gemini-2.0-flash"; // Using Gemini Pro model
...
function buildPrompt(history) {
    let prompt = `You are an expert fantasy RPG Dungeon Master. Follow these rules:
    ...
    prompt += "\nWhat happens next? Describe in vivid detail, advancing the story.";
    return prompt;
}
...
function cleanAIResponse(response) {
    ...
    let cleanText = response.replace(/DM:\s*/g, '');
...
async function generateInitialScene(campaignName, storyline) {
    ...
    try {
        const initialPrompt = `You are an expert fantasy RPG Dungeon Master starting a new campaign called "${campaignName}".
        ...
        Describe the opening scene in 3-5 detailed sentences.`;
        
        const response = await callGeminiAPI(initialPrompt);
